...
const ReauthenticateModal = ({
  ...
  // OAuth users don't have a password to submit — we pass null and let
  // the parent handle the popup flow after this modal closes.
  const handleSubmit = (e) => {
    e.preventDefault();
    onReauthenticate(hasPasswordAuth ? password : null);
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div className="bg-white/90 dark:bg-black backdrop-blur-md rounded-2xl shadow-2xl border border-gray-300/40 dark:border-gray-700/50 w-full max-w-md p-6 sm:p-8 transform animate-in zoom-in-95 duration-300 ease-out">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed">
          {hasPasswordAuth
            ? "Just making sure it's really you. Enter your current password to permanently delete your account."
            : `You signed in with ${
                authProvider === "google.com" ? "Google" : "GitHub"
              }. To confirm this is you, we'll open a quick verification with ${
                authProvider === "google.com" ? "Google" : "GitHub"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            {/* For OAuth users the password check is skipped — blank password
                is only a blocker when they actually have one to enter. */}
            <button
              type="submit"
              disabled={loading || (hasPasswordAuth && !password.trim())}
